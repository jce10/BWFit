...
def plot_upper_limit(
    ...
    *,
    ...
    """Plot one upper limit value per fit range."""
    quality = (
        df[["theta", "counts_err"]]
    ...
    ax.plot(quality["theta"], quality["counts_err"], "o-")
    # ax.set_xlabel(r"$\theta_{\mathrm{lab}}$ (deg)")
    ax.set_xlabel(r"fit range")
    ax.set_ylabel(r"Upper limit on counts")
...
def plot_interference_phase(
    df: pd.DataFrame,
    *,
    output_path: Path,
    show: bool,
) -> None:
    """Plot the fitted interference phase and uncertainty versus angle."""
    phase_data = (
        df[["theta", "phase", "phase_err"]]
        .drop_duplicates(subset=["theta"])
        .sort_values("theta")
    )
    ...
    ax.set_xlabel(r"$\theta_{\mathrm{lab}}$ (deg)")
    ax.set_ylabel("Interference phase (rad)")
    ax.set_ylim(-pi/2, pi/2)
    ax.set_title("Interference phase, δ, versus angle")
    ax.grid(alpha=0.3)
    fig.tight_layout()
    fig.savefig(output_path, dpi=300, bbox_inches="tight")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--log-counts",
        action="store_true",
        help="Use a logarithmic y-axis for the counts figure",
    )
    parser.add_argument(
        "--show",
        action="store_true",
...
def main() -> None:
    ...
    plot_parameter(
        df,
        set_column="counts_set",
        fit_column="counts_fit",
        error_column="counts_err",
        ylabel="Integrated counts",
        title="State integrated counts versus angle",
        output_path=args.output_dir / f"counts_vs_angle.{suffix}",
        show=args.show,
        log_y=args.log_counts,
    )

    plot_upper_limit(
        df,
        output_path=args.output_dir / f"upper_limit_counts_vs_angle.{suffix}",
        show=args.show,
    )
    ...
    plot_interference_phase(
        df,
        output_path=args.output_dir / f"interference_phase_vs_angle.{suffix}",
        show=args.show,
    )
